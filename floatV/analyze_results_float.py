...
def load_instance_user_needs(instances_dir, instance_name):
    ...
    返回 dict {user_id: need}（need 为 float），若失败返回 None。
    """
    ...
    try:
        ...
        for u in inst.get("users", []):
            ...
            if uid is not None and need is not None:
                try:
                    needs[str(uid)] = float(need)
                except Exception:
                    # fallback try converting from string with possible commas etc.
                    try:
                        needs[str(uid)] = float(str(need).replace(",", ""))
                    except Exception:
                        needs[str(uid)] = None
        return needs
    except Exception:
...
def load_instance_total_demand(instances_dir, instance_name):
    """
    尝试从 instances_dir 中加载名为 instance_name.json 的实例并返回 total_demand（float）
    若不存在或读取失败，则返回 None
    """
    p = Path(instances_dir) / f"{instance_name}.json"
    if not p.exists():
    ...
    try:
        ...
        for u in inst.get("users", []):
            try:
                total += float(u.get("need", 0.0))
            except Exception:
                try:
                    total += float(str(u.get("need", 0.0)).replace(",", ""))
                except Exception:
                    total += 0.0
        return float(total)
    except Exception:
    ...
    if allocs is None:
        ...
        for k, v in result_dict.items():
            if isinstance(v, dict):
                # heuristics: dict of user_id -> list/tuple/dict
...
def compute_satisfaction_stats(alloc_map, user_needs_map, total_users_from_meta=None):
    ...
    给定 alloc_map: user_id->allocated_amount (float)
          user_needs_map: user_id->need (float)  或 None
    计算满足率 >=100%, >=80%, >=60% 的用户数量与占比。
    如果 user_needs_map 为 None 则返回 None 值的字段。
    ...
    for uid, alloc in alloc_map.items():
        ...
        if need is None:
            # unknown need; skip counting for ratios
            continue
        # treat zero need as skip
        try:
            need_f = float(need)
        except Exception:
            continue
        if need_f <= EPS:
            continue
        known_users += 1
        ratio = float(alloc) / float(need_f) if need_f > EPS else 0.0
        if ratio >= 1.0 - 1e-12:
            full += 1
        if ratio >= 0.8 - 1e-12:
            p80 += 1
    ...
    # denom for share: prefer total_users_from_meta if provided and >0, else known_users
    denom = None
...
def parse_result_file(path, instances_dir=None):
    ...
    def analyze_method(result_dict):
        ...
        # unserved count/share
        if alloc_map is None:
            unserved_count = None
            unserved_share = None
        else:
            unserved_count = sum(1 for v in alloc_map.values() if (v is None or float(v) <= 0.0 + EPS))
            denom = None
            try:
...
def plot_runtime_vs_pref_improved(df, plotdir):
    ...
    colors = {"mcmf": "C0", "greedy": "C1", "lp": "C2", "warm_mcmf": "C3", "hungarian": "C4", "pso": "C5"}
...
def plot_comparisons(df, plotdir, max_instances_for_bar=20):
    ...
    if "mcmf_fulfillment" in df.columns and df["mcmf_fulfillment"].notnull().any():
        ful_cols = [c for c in ["mcmf_fulfillment", "greedy_fulfillment", "lp_fulfillment", "warm_mcmf_fulfillment", "hungarian_fulfillment", "pso_fulfillment"] if c in df.columns and df[c].notnull().any()]
        if len(ful_cols) > 0:
    ...
    # Scatter runtime vs total_pref for each method
    fig, ax = plt.subplots(figsize=(8, 6))
    plotted = False
    colors = {"mcmf": "C0", "greedy": "C1", "lp": "C2", "warm_mcmf": "C3", "hungarian": "C4", "pso": "C5"}
    for m in available:
